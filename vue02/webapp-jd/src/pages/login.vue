<template>
    <div class="cont">
      <header>
        <i class="iconfont icon-lunbotuqiehuanzuo"></i>
        <span>京东登录</span>
      </header>
      <section class="sec-form">
        ...
        <form class="form">
          <div class="user">
            <input type="text" placeholder="手机号/邮件地址" v-model="loginUser.username">
            <span></span>
            <p class="user-error"></p>
          </div>
          <div class="pass">
            <input type="password" placeholder="密码" v-model="loginUser.userpass">
            <i class="iconfont icon-yincangmima"></i>
            <p class="user-error"></p>
          </div>
          <div class="message">
            <div class="left"><a href="">短信验证码登录</a></div>
            <div class="right"><input type="checkbox"><label for="">记住账号</label></div>
          </div>
          <div class="login">
...
<script>
    export default {
      ...
      beforeRouteEnter(to,from,next){
        ...
        next();
        if(window.localStorage.getItem("exist")){
          next({path:"/shopcarts"})
        }
      },
      methods:{
          ...
          login(){
            window.localStorage.setItem("exist","ok")
            this.$router.push({path:"/shopcarts"})
            // this.$store.dispatch({type:"LOGIN",data:1})
            // console.log(this.loginUser.username)
            // console.log(this.loginUser.userpass)
          }
      }
    }
</script>
